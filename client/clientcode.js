if (Meteor.isClient) {
    ...
    Template.home.events({
        'click #btnSetUsername': function () {
            ...
            if ($('#txtSetUsername').val() != '') {
                ...
                $('#txtSetUsername').parent('p').append('<h2>Your username is ' + Session.get('Username') + 'and PlayerId is ' + Session.get('PlayerId') + '</h2><a href="/game/5">Go to game</a>');
            }
    ...
    Template.hello = function () {
        ...
        $.each(camelArray, function (index) {
            ...
            console.log(this);
            console.log("t" + [currentLocations[index] - game.Players[index].Distance * 10 , ys[index]] + "s" + [0.25]) ;
            // this.an
        });
    ...
    Template.hello.rendered = function () {
        // if (!this._rendered) {
        //   this._rendered = true;
        game = Games.findOne({}, {GameId: Session.get("GameId")});
...
// All animation for the ballThrowLocation
    ...
    function ballGoesInHole(hole) {
        var testingshit = Games.findOne({}, {GameId: Session.get("GameId")});
        testingshit.Players[Session.get("PlayerId")].Distance =  parseInt(hole.nr);



        Games.update(testingshit._id,testingshit);
        game = testingshit ;
        //game.Players[Session.get("PlayerId")].Distance = parseInt(hole.nr);
        moveCamels();
        console.log(game);
    ...
    function moveCamels() {
        ...
        $.each(camelArray, function (index) {
            ...
            console.log([currentLocations[index] - game.Players[index].Distance * 10 , ys[index]] + "s" + [0.25]) ;
            // this.an
        });
